...
string hasData(string s) {
  auto found_null = s.find("null");//find the index, first 
  auto b1 = s.find_first_of("[");//find the first letter of string, index
  auto b2 = s.find_first_of("]");
  if (found_null != string::npos) {//'null' is founded
    return "";
  } else if (b1 != string::npos && b2 != string::npos) {
    return s.substr(b1, b2 - b1 + 1); //substr for b1, length:b2-b1+1;
  }
  return "";
}
...
int main() {
  ...
  double sigma_pos [3] = {0.3, 0.3, 0.01};//measurement noise
  // Landmark measurement uncertainty [x [m], y [m]]
  double sigma_landmark [2] = {0.3, 0.3};//measurement noise
